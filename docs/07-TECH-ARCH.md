...
- **Перенос персонажа — только через `World/Teleport`.** Поляны и туннели стоят за тысячи
  студов от площади; при `character:PivotTo` напрямую пол у клиента ещё не подгружен, и ребёнок
  падает в пустоту. `Teleport.to` ставит персонажа на якорь, просит `RequestStreamAroundAsync`
  и отпускает якорь, когда место пришло. Он же ведёт «безопасную точку» и возвращает туда
  провалившегося (страж падения). В месте включён `StreamingIntegrityMode = PauseOutsideLoadedArea`.
- Появился заново (новый персонаж) — идущее занятие обрывается (`ChallengeService`), иначе
  пады и порталы молча не запускались бы.
...
- **Мелкие шарики чужих участков издалека не рисуются.** Шар в Roblox — сотня
  с лишним треугольников, а на застроенном участке их около 120 (глазки, цветы,
  лампочки). Сервер метит шары до 1,6 студа тегом `PlotDetail`, клиент
  (`DetailLodController`) прячет их у участков дальше 95 студов от камеры.

### 8.1. Замер

Планшета под рукой нет, поэтому меряется то, что его грузит, — в настоящем Play:

```bash
python tools/playtest/run.py perf               # детали по разделам мира и кадр в четырёх точках
ZZ_ONLY=tris python tools/playtest/run.py perf  # сколько треугольников даёт каждый вид деталей
```

Сценарий застраивает все 20 участков до конца — худший случай для площади.
Результаты 17.09.2026 (Studio на ПК, 60 FPS упирается в предел кадров):

| Точка | Треугольников до | после | Источников света до | после |
|---|---:|---:|---:|---:|
| От точки появления | 354 тыс. | 231 тыс. | 22 | 3 |
| Над площадью | 520 тыс. | 353 тыс. | 22 | 3 |
| Парк | 264 тыс. | 143 тыс. | 22 | 3 |
| Между участками | 88 тыс. | 72 тыс. | 22 | 3 |

Откуда были треугольники: 2280 шаров на 19 участках — около 305 тыс., 1653 цилиндра —
около 54 тыс., меши моделей Store на площади — около 88 тыс., шары декора площади — 26 тыс.
Лишние 19 источников света давали звёзды на золотых крышах — теперь они светятся неоном.

Что остаётся выше бюджета: деталей в радиусе 250 студов 3,6–5 тыс. при бюджете ~800
(отрисовка группирует одинаковые детали — вызовов отрисовки 65–260), и 1667 SurfaceGui
у буквенных заборов на всех участках сразу (гаснут дальше 80 студов). Проверять
на настоящем планшете — пункт чек-листа «30 FPS на дешёвом планшете».
