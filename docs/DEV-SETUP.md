...
1. `ChallengeService` + испытание «Буквопад» целиком: сервер строит задание по seed,
   валидирует тапы, начисляет.
2. Клиентская часть испытания: падающие буквы, HUD, обратная связь.
3. Хаб, Башня, магазин на 12 предметов (`ShopService`, `CosmeticsService`).
